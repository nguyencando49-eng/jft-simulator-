...
export async function GET(req:Request){ try{ await requireAuth(req,'admin'); const id=new URL(req.url).searchParams.get('id')??'JFT-PRACTICE-A1-001'; const repo=getRepository(); return NextResponse.json({ok:true,draft:await repo.getExamDraft(id),versions:await repo.listExamVersions(id)}); }catch(e){return apiError(e);} }
