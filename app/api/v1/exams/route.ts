...
import { loadReleaseBankForDraft } from '@/lib/server/release-bank-provider';
...
export async function POST(req:Request){ try{ await requireAuth(req,'admin'); const {examId}=await req.json() as {examId:string}; const repo=getRepository(); const draft=await repo.getExamDraft(examId); if(!draft)return NextResponse.json({ok:false,error:'Exam draft not found'},{status:404}); const release=await loadReleaseBankForDraft(draft); const bank=release.source==='release'?release.questions:await repo.listQuestions(); const versions=await repo.listExamVersions(examId); const generated=generateExamVersion(draft,bank,Math.max(0,...versions.map(v=>v.version))+1); if(!generated.ok)return NextResponse.json(generated,{status:422}); await repo.saveExamVersion(generated.version); return NextResponse.json({ok:true,version:generated.version,release:{source:release.source,level:release.releaseLevel,version:release.releaseVersion,hash:release.releaseHash}},{status:201}); }catch(e){return apiError(e);} }
