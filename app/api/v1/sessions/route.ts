...
export async function POST(req:Request){
  try{
    ...
    if(active){const rules=version.rules?.map(r=>({section:r.section,allowBack:r.allowBack}))??Array.from(new Set(version.questions.map(q=>q.snapshot.section))).map(section=>({section,allowBack:section!=='listening'}));const questions=version.questions.map(f=>toCandidateQuestion(f.snapshot));return NextResponse.json({ok:true,session:active,exam:{id:version.id,title:version.title,durationMinutes:version.durationMinutes,rules,questions},resumed:true});}
