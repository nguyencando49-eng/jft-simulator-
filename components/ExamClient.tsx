...
function Frame({timer,section,progress,children}:{timer:string;section?:string;progress?:string;children:React.ReactNode}){
  ...
  const percent=parts&&parts.length===2&&parts[1]>0?Math.max(0,Math.min(100,(parts[0]/parts[1])*100)):0;
  return <main className="exam-app exam-app-pro"><header className="exam-top exam-top-pro"><div className="exam-brand"><span className="exam-brand-mark">日</span><div><b>JFT Practice</b><span>Bài luyện tập không chính thức</span></div></div>{section&&<div className="exam-context"><span>{section}</span><b>{progress}</b></div>}<div className="exam-timer"><span>Thời gian còn lại</span><b>{timer}</b></div></header>{section&&<div className="exam-global-progress" aria-hidden="true"><span style={{width:percent+'%'}}/></div>}<div className="mobile-cbt-note">Giao diện đã tối ưu cho mobile. Với phần Nghe hiểu, nên dùng tai nghe.</div>{children}</main>;
}
