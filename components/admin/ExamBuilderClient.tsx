...
export default function ExamBuilderClient(){
  ...
  const [productionReport,setProductionReport]=useState<ProductionReleaseReport|null>(null),[productionReady,setProductionReady]=useState(false),[productionPublished,setProductionPublished]=useState<string[]>([]);
  ...
  useEffect(()=>{
    void Promise.all([adminApi.exam(),adminApi.questions(),adminApi.productionRelease()])
      .then(([exam,questions,release])=>{
        ...
        setProductionReport(release.report);setProductionReady(release.ready);setProductionPublished(release.publishedVersionIds);
      })
  ...
  async function publishProduction(){try{
    ...
    setProductionReport(result.report);setProductionPublished([...result.published,...result.skipped]);setProductionReady(true);
  ...
  return <>
    ...
    <section className="admin-panel production-release-card" data-testid="production-3000-release"><div className="panel-head"><div><h2>Production 3000</h2><p>Phát hành đồng bộ ngân hàng 3.000 câu và 3 đề chuẩn A1 / A2.1 / A2.2. Mỗi đề 48 câu, 60 phút, snapshot bất biến.</p></div><span className={`readiness-pill ${productionReady?'ready':productionReport?'review':'blocked'}`}>{productionReady?'ĐÃ PHÁT HÀNH':productionReport?'SẴN SÀNG PHÁT HÀNH':'ĐANG KIỂM TRA'}</span></div>
      ...
      <div className="home-actions"><button className="primary" disabled={releasing||productionReady} onClick={()=>void publishProduction()}>{releasing?'Đang đồng bộ & phát hành…':productionReady?'Production 3000 đã phát hành':'Phát hành Production 3000'}</button></div></>}
    </section>
