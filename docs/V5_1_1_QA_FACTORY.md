...
The Factory QA sequence also preserves independent gate evidence: QA1 content review is followed by the blinded QA2 Answer Oracle, QA3 Japanese Naturalness, QA4 Curriculum Grounding, QA5 JFT Alignment, and QA6 Difficulty Calibration. QA2 independently derives defensible answers before deterministic comparison with the declared key. QA3 separately evaluates grammar, collocation, register, context, spoken/written fit, dialogue flow, and every choice. QA4 independently extracts required/supporting/incidental knowledge, maps only approved KnowledgeUnits, and applies deterministic negative-evidence and hard-fail policy. QA5 first classifies the learner ability without seeing the declared section/category/Can-do/task purpose, then application code compares that classification with the declared target. QA6 creates a blinded multidimensional difficulty profile, after which application code calculates a versioned internal score, estimates the canonical level, and combines only sufficiently sampled aggregate performance as human-review evidence. A later specialized failure overrides earlier passes; calibrated-looking difficulty cannot rescue an invalid answer, curriculum boundary, or assessment target. See `docs/qa/QA2_ANSWER_ORACLE.md`, `docs/qa/QA3_JAPANESE_NATURALNESS.md`, `docs/qa/QA4_CURRICULUM_GROUNDING.md`, `docs/qa/QA5_JFT_ALIGNMENT.md`, and `docs/qa/QA6_DIFFICULTY_CALIBRATION.md`.
