...
export async function persistGeneratedAudio(result:TtsResult,jobId:string,candidateId:string){
  ...
  const upload=await fetch(`${url.replace(/\/$/,'')}/storage/v1/object/${encodeURIComponent(bucket)}/${path}`,{method:'POST',headers:{apikey:key,Authorization:`Bearer ${key}`,'Content-Type':result.contentType,'x-upsert':'true'},signal:AbortSignal.timeout(Number(process.env.STORAGE_REQUEST_TIMEOUT_MS||30000)),body:Buffer.from(result.bytes)});
