...
export class DeterministicJftContentQaJudge {
  ...
  judge(q:QaQuestion,evidence:QaEvidence={}):JftContentQaResultV1{
    ...
    if(derived===null)issues.push(issue('INSUFFICIENT_EVIDENCE','WARNING','The deterministic screen could not derive one option from surface-form rules alone.','Surface heuristics are intentionally non-authoritative for semantic Japanese items.','Require QA2 Answer Oracle or human review before approval.'));
    ...
    const canDoWeak=false;
    const category=declaredCategory(q),actual=actualCategory(q);
    const allowedCategory:Record<SectionId,string[]>={script_vocabulary:['word_meaning','word_usage','kanji_reading','kanji_meaning_usage'],conversation_expression:['grammar','expression'],listening:['conversation','shop_public','announcement_instruction'],reading:['content_comprehension','information_search']};
    if(category&&!allowedCategory[q.section].includes(category))issues.push(issue('CATEGORY_MISMATCH','MAJOR',`Declared category ${category} is invalid for ${q.section}.`,'Category metadata must belong to the declared section taxonomy.','Return the item to planning with a valid category for this section.'));
    ...
    const estimated=q.level,levelMatch='MATCH' as const;
    ...
    const scores={japaneseNaturalness:naturalness,canDoAlignment:canDoWeak?4:12,situationRealism:q.prompt.length>20?13:7,answerUniqueness:derived===null?10:derived!==null&&answerMatch&&uniqueChoices?15:5,distractorQuality:distractorWeak?3:7,levelAppropriateness:levelMatch==='MATCH'?9:6,categoryAlignment:category&&normalize(category)===normalize(actual)?5:2,originality:evidence.sourceSimilarityScore===undefined?2:evidence.sourceSimilarityScore<.72?5:0,metadataCompleteness:metadataComplete?5:2,total:0};
    scores.total=Object.values(scores).reduce((sum,value)=>sum+value,0);
    const hardFail=hardCodes.size>0;const assessmentValue=leaked||distractorWeak?'LOW':'MEDIUM';
    let verdict:Verdict=hardFail||scores.total<80?'FAIL':scores.total<90?'REVIEW':'PASS';
    const confidence:Confidence=derived!==null&&unit?'HIGH':unit?'MEDIUM':'LOW';if(confidence!=='HIGH'&&verdict==='PASS')verdict='REVIEW';
