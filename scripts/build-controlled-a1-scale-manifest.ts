...
const target=1320,batchSize=20;
...
const manifest={version:'CONTROLLED_A1_SCALE_MANIFEST_V2',generatedAt:new Date().toISOString(),target,batchSize,generated,remaining,remainingBatchCount:Math.ceil(remaining/batchSize),reviewPolicy:'CUMULATIVE_REVIEW_AT_1320',publicationPolicy:'NO_AUTO_APPROVAL_OR_PUBLISH',providerRequirement:{provider:'azure-openai',model:'jft-gpt-5-mini',configuredLocally:Boolean(process.env.AI_FACTORY_ENDPOINT&&process.env.AI_FACTORY_API_KEY&&process.env.AI_FACTORY_MODEL)},artifacts:rows};
