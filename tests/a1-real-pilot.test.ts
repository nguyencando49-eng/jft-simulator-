...
describe('A1 lesson 3 real-curriculum pilot',()=>{
  ...
  it('requires fixed audio before listening candidates pass QA',()=>{const listening=a1Lesson03Candidates.filter(q=>q.section==='listening');expect(listening.every(q=>q.audioScript&&q.audioSrc?.startsWith('/audio/'))).toBe(true);for(const q of listening){const report=runQuestionQa({...q,version:1,status:'review',source:'ai',createdAt:'x',updatedAt:'x'});expect(report.passed,q.id).toBe(true);expect(report.issues.some(issue=>issue.code==='audio_required')).toBe(false);const wav=readFileSync(join(process.cwd(),'public',q.audioSrc!));expect(wav.toString('ascii',0,4),q.id).toBe('RIFF');expect(wav.toString('ascii',8,12),q.id).toBe('WAVE');}});
});
