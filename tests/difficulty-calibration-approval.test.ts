...
async function fixture(audioPending=false){
  const scenes=[
    {noun:'電車',prompt:'電車は何時に出ますか。',choices:['8時','9時','10時','11時'],answer:2,script:'お知らせです。電車は10時に出ます。'},
    {noun:'バス',prompt:'バスは何時に出ますか。',choices:['6時','7時','8時','9時'],answer:1,script:'お知らせです。バスは7時に出ます。'},
    {noun:'会議',prompt:'会議は何時からですか。',choices:['1時','2時','3時','4時'],answer:2,script:'お知らせです。会議は3時からです。'},
    {noun:'受付',prompt:'受付は何時からですか。',choices:['7時','8時','9時','10時'],answer:2,script:'ご案内です。受付は9時からです。'},
  ];const scene=scenes[fixtureOrdinal++%scenes.length];
  ...
  await repo.saveKnowledgeUnits([{id:unitId,sourceDocumentId:sourceId,sourceChunkIds:[chunkId],topic:'交通',situation:'駅',level:'A1',canDo:'Understand a simple spoken announcement or instruction.',grammar:['～ます'],vocabulary:['電車','バス','会議','受付','時間','時','出ます'],kanji:['電車','会議','受付','時間','時'],expressions:[],keyKnowledge:['開始時間を理解する'],skills:['listening'],confidence:.98,status:'approved',createdAt:now,updatedAt:now,provider:'synthetic-test',promptVersion:'test-v1'}]);
  const question={id:questionId,section:'listening' as const,type:'audio_choice' as const,level:'A1' as const,instruction:'音声を聞いて答えてください。',prompt:scene.prompt,choices:scene.choices,answer:scene.answer,explanationVi:'Thông tin thời gian được nêu trực tiếp trong âm thanh.',audioSrc:audioPending?undefined:'data:audio/wav;base64,AAAA',tags:['transportation'],version:1,status:'review' as const,source:'ai' as const,createdAt:now,updatedAt:now};
  const audioScript=scene.script;const staleInput=buildDifficultyCalibrationInput(question,{audioScript,audioEvidence:{available:true,durationMs:2_000}});const staleAnalysis={...qa6Analysis(staleInput,.15),acousticAssessment:'ASSESSED' as const};
...
function stubGates(unitId:string,chunkId:string,qa6Mode:'extreme'|'invalid'='extreme'){
  ...
  vi.stubGlobal('fetch',vi.fn(async(url,init)=>{const payload=JSON.parse(String((init as RequestInit).body));
    ...
    if(String(url).includes('qa7.test'))return new Response(JSON.stringify({qaVersion:ORIGINALITY_DUPLICATE_PROMPT_VERSION,questionId:payload.input.questionId,confidence:'HIGH',relationships:payload.input.comparisons.map((item:any)=>({comparisonId:item.id,kind:item.kind,semanticRisk:'NONE',relationship:'DISTINCT',evidence:'Synthetic QA6 isolation marks this comparison distinct.'}))}),{status:200,headers:{'content-type':'application/json'}});
...
describe('QA6 Factory checkpoint integration',()=>{
  ...
  it('reruns QA6 after Listening TTS refresh instead of retaining stale PASS evidence',async()=>{
    ...
    expect(result.candidate.audio?.status).toBe('ready');expect(result.candidate.difficultyCalibrationQa).toMatchObject({verdict:'FAIL',hardFail:true,provider:'http-difficulty-calibration'});expect(result.candidate.difficultyCalibrationQa?.checkedAt).not.toBe(stale.checkedAt);expect(result.candidate.originalityDuplicateQa).toMatchObject({promptVersion:ORIGINALITY_DUPLICATE_PROMPT_VERSION});expect(result.candidate.qa.passed).toBe(false);
  });
