...
const make=(n:number):QuestionRecord=>({id:`Q${n}`,section:'reading',type:'choice',level:'A2.1',instruction:'',prompt:`P${n}`,choices:['a','b','c','d'],answer:0,explanationVi:'x',tags:[],version:1,status:'approved',source:'original',createdAt:'x',updatedAt:'x'});
...
describe('seeded exam generation',()=>{
  ...
  it('freezes displayed choice order separately from the canonical answer',()=>{const bank=Array.from({length:20},(_,i)=>make(i+1));const generated=generateExamVersion(draft,bank,1);expect(generated.ok).toBe(true);if(!generated.ok)return;expect(generated.version.questions.map(item=>item.snapshot.answer).sort()).toEqual([0,1,2,3,].concat([0]).sort());for(const item of generated.version.questions){expect(item.canonicalSnapshot?.answer).toBe(0);expect(item.choicePermutation?.permutation[item.snapshot.answer]).toBe(0);expect(item.snapshot.choices[item.snapshot.answer]).toBe(item.canonicalSnapshot?.choices[0]);}});
});
